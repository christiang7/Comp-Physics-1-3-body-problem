...
*Star-Trek-Dreikorperproblem.tex*
```tex
...
\chapter{Grundlagen}\label{ch:grd}
...
\text{mit } h&=t^{n+1}-t^{n} \text{ und }\vec {z^n}=(z_1^n,z_2^n,z_3^n,z_4^n)^T\notag
\end{align}
...
&\vec z^{n+1}= \vec z^{n} + h ( \frac{1}{6} \vec k_1 +  \frac{4}{6}\vec k_2 + \frac{1}{6} \vec k_3 ) \\
&\text{mit}\notag \\
&\left\{
\begin{array}{l l l}
\vec k_1=\vec f(\vec z^n)\\
\vec k_2=\vec f(\vec z^{n}+\frac{h}{2} \vec k_{1})\\
\vec k_3=\vec f(\vec z^{n}-h \vec k_{1}+2h \vec k_{2})
\end{array}
\right.\ \notag
\end{align}
...
&\mathrm{if} (|x|>2)\\
&\mathrm{if} (|y|>2)\\
&\mathrm{if}((x+\mu)^2+y^2<R_1^2) \\
&\mathrm{if}(((x-(1-\mu))^2+y^2<R_2^2) \\
...
\chapter{Mathematische Grundlagen und Algorithmen}
...
*make.sh*
```bash
noweb.py -RStar-Trek-Dreikorperproblem.tex Star-Trek-Dreikorperproblem.md > Star-Trek-Dreikorperproblem.tex && lualatex -interaction=nonstopmode -shell-escape Star-Trek-Dreikorperproblem.tex && lualatex -interaction=nonstopmode -shell-escape Star-Trek-Dreikorperproblem.tex && date && xdg-open Star-Trek-Dreikorperproblem.pdf 2>/dev/null 
```
