...
\begin{align}
\dot x & =\frac{dx}{dt}=\frac{\partial H}{\partial p_x}=p_x+y\cr
\dot y & =\frac{dy}{dt}=\frac{\partial H}{\partial p_y}=p_y-x \label{Bew_glg_x}\\
\dot p_x & =\frac{dp_x}{dt}=-\frac{\partial H}{\partial x}=p_y-x+\Omega_x \cr
\dot p_y & =\frac{dp_y}{dt}=-\frac{\partial H}{\partial y}=-p_x-y+\Omega_y \label{Bew_glg_p}\\
...
\ddot x & =\dot p_x+\dot y=p_y-x+\Omega_x+\dot y\cr
...
\ddot x & =\dot y+x-x+\Omega_x+\dot y=2\dot y+\Omega_x\cr
...
\dot z_1 & =z_3\cr
\dot z_2 & =z_4\cr
\dot z_3 & =2z_4+z_1-\frac{(1-\mu)\cdot(z_1+\mu)}{r_1^3}-\frac{\mu\cdot(z_1-1+\mu)}{r_2^3}=2z_4+\Omega_{z_1}(z_1,z_2)\cr
...
\text{mit} & \notag \\
\vec{k}_1 & =\vec{f}\left(\vec{z}^{\,n}\right)\cr
\vec{k}_2 & =\vec{f}\left(\vec{z}^{\,n}+\frac{h}{2} \vec{k}_{1}\right)\cr
...
\begin{array}{l}
\vec{p}^{\,\,n+\frac{1}{2}} = \vec{p}^{\,\,n} + \frac{h}{2}  \nabla_{\vec{x}}\, \Omega (\vec{x}^{\,n})\\
\vec{x}^{\,\,n+1} = \vec{x}^{\,\,n} + h \nabla_{\vec{p}} \, H_1 (\vec{p}^{\,n},\vec{x}^{\,n})\\
...
&\mathbf{if} (|x|>2)\\
&\mathbf{if} (|y|>2)\\
&\mathbf{if}\left((x+\mu)^2+y^2<R_1^2\right) \\
&\mathbf{if}\left(((x-(1-\mu))^2+y^2<R_2^2\right) \\
...
&\mathbf{if} (j\ge 2)\{\notag\\
&\quad	\mathbf{if}(j_E\ge 2) \text{ Abbruch};\\
&\}\notag
\end{align}
...
\left\{
\begin{array}{l l}
z_k=\frac{x_{k-1}-y_{k-1}}{2}&, \forall \, k \in \mathbb{Z^+}\\
x_k=z_k, y_{k}=y_{k-1}&, \text{falls} \, \mathop{\mathrm{sgn}}(f(z_k))=\mathop{\mathrm{sgn}}(f(x_{k-1}))\\
y_k=z_k, x_{k}=x_{k-1}&, \text{sonst}
\end{array}
\right.\
\end{align}\\
...
\mathbf{if}(w_\mathrm{step, neu}\le\epsilon)
\end{align}
...
*make.sh*
```bash
noweb.py -Rtest.tex test.md > test.tex && lualatex -interaction=nonstopmode -shell-escape test.tex && lualatex -interaction=nonstopmode -shell-escape test.tex && notify-send -a "Compilation $(date)" fertig && xdg-open test.pdf 2>/dev/null 
```
